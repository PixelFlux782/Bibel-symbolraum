  ...
  return (
    <section className="relative flex min-h-screen items-end overflow-hidden px-5 pb-14 pt-32 sm:px-8 lg:px-14">
      <div className="pointer-events-none absolute inset-0 z-0">
        ...
        <div className="absolute left-0 top-0 h-full w-full bg-[radial-gradient(circle_at_22%_24%,rgba(216,195,148,0.14),transparent_26%),radial-gradient(circle_at_72%_66%,rgba(88,148,176,0.13),transparent_24%)]" />
      ...
      >
        ...
        <h1 className="max-w-4xl font-serif text-5xl italic leading-[1.04] text-foreground-strong sm:text-7xl lg:text-8xl">
          {title}
        </h1>
        <p
          ...
          >
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="group border border-white/10 bg-black/20 px-5 py-4 text-left backdrop-blur-md transition duration-300 hover:border-gold/40 hover:bg-white/10"
              >
                {link.meta ? (
                ...
                <span className="mt-4 block h-px w-10 bg-gold/45 transition-all duration-300 group-hover:w-20" />
              </Link>
            ))}
          </div>
        ) : null}
