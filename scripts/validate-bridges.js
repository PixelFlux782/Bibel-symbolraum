...
function validateWaterReferenceBridge() {
  ...
  [
    ...
    failIf(waterBridge?.ctaLabels.roomReturn === "In den Wasserraum zurueckkehren", "Water room return CTA label missing or changed.", waterBridge?.ctaLabels.roomReturn),
    ...
    failIf(sparseWaterLinks.some((link) => link.label === "In den Wasserraum zurueckkehren"), "Sparse water reflection gets room fallback.", sparseWaterLinks),
...
function validateLightPreparedBridge() {
  ...
  [
    ...
    failIf(sparseLightLinks.some((link) => link.label === "In den Lichtraum zurueckkehren"), "Light reflection return link missing: sparse light reflection gets room fallback.", sparseLightLinks),
