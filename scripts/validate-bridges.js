...
function validateWaterReferenceBridge() {
  ...
  [
    ...
    failIf(waterBridge?.ctaLabels.roomReturn === "In den Wasserraum zurückkehren", "Water room return CTA label missing or changed.", waterBridge?.ctaLabels.roomReturn),
    ...
    failIf(sparseWaterLinks.some((link) => link.label === "In den Wasserraum zurückkehren"), "Sparse water reflection gets room fallback.", sparseWaterLinks),
  ...
  if (waterBridge && waterBridge.movement.join(" -> ") !== "Symbolnetz -> Codex -> Raum -> persönliche Spur -> Mein Pfad") {
    warnings.push({
...
function validateLightPreparedBridge() {
  ...
  [
    ...
    failIf(sparseLightLinks.some((link) => link.label === "In den Lichtraum zurückkehren"), "Light reflection return link missing: sparse light reflection gets room fallback.", sparseLightLinks),
    ...
    failIf(pathLightLinks.some((link) => link.key === "trace" && link.href === "/codex/genesis-1-3" && link.label === "Zur Lichtspur zurückkehren"), "Light reflection return link missing: path reflection should return to its Codex anchor.", pathLightLinks),
